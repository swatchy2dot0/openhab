	...
	public String getItemName(String id) {
		for (Entry<String, BindingConfig> entry : bindingConfigs.entrySet()) {
			...
			if(id.equals(cfg.id + "." + cfg.type)) {
				...
				logger.info("Found new Lacrosse-Device ID: " + id.substring(0, 3));
			}
		}
	...
	public void processBindingConfiguration(String context, Item item,
		...
		for (String set : bindingConfig.trim().split(",")) {
			...
			if (configParts.length > 2) {
				throw new BindingConfigParseException("lacrosse binding configuration must not contain more than two parts");
			}
